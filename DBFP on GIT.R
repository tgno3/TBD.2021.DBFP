...
# Run Malmquist
# Incu
# res.mq.s1.inc <- roc.malmquist(df.br.7.3d[type == 1, id.x.s1.m,], df.br.7.3d[type == 1, id.y.s1.m,], tm, dm.s1, rts.s1, g = g.m[type == 1,,], wd = wd)
# res.mq.s2.inc <- roc.malmquist(df.br.7.3d[type == 1, id.x.s2.m,], df.br.7.3d[type == 1, id.y.s2.m,], tm, dm.s2, rts.s2, orientation)

# Nor.sm
res.mq.s1.nsm <- roc.malmquist(df.br.7.3d[type == 2, id.x.s1.m,], df.br.7.3d[type == 2, id.y.s1.m,], tm, dm.s1, rts.s1, g = g.m[type == 2,,], wd = wd)
res.mq.s2.nsm <- roc.malmquist(df.br.7.3d[type == 2, id.x.s2.m,], df.br.7.3d[type == 2, id.y.s2.m,], tm, dm.s2, rts.s2, orientation)

# Nor.lg
# res.mq.s1.nlg <- roc.malmquist(df.br.7.3d[type == 3, id.x.s1.m,], df.br.7.3d[type == 3, id.y.s1.m,], tm, dm.s1, rts.s1, g = g.m[type == 3,,], wd = wd)
# res.mq.s2.nlg <- roc.malmquist(df.br.7.3d[type == 3, id.x.s2.m,], df.br.7.3d[type == 3, id.y.s2.m,], tm, dm.s2, rts.s2, orientation)
...
# DMU of interest
data.frame(DMU   = 1:length(names(id.7)[which(type == 2)]),
           Br.No = names(id.7)[which(type == 2)])
id.doi   <- c(13, 20)
df.s1.fs <- data.frame(subset(res.mq.s1.nsm$fs, DMU %in% id.doi),
                       Name = names(id.7)[which(type == 2)][id.doi])
df.s1.mi <- data.frame(subset(res.mq.s1.nsm$mi, DMU %in% id.doi),
                       Name = names(id.7)[which(type == 2)][id.doi])
df.s2.fs <- data.frame(subset(res.mq.s2.nsm$fs, DMU %in% id.doi),
                       Name = names(id.7)[which(type == 2)][id.doi])
df.s2.mi <- data.frame(subset(res.mq.s2.nsm$mi, DMU %in% id.doi),
                       Name = names(id.7)[which(type == 2)][id.doi])
